...
Fixity facilitates checking across all AIPs in the Storage Service, or
spot-checking of individual packages.

Check Fixity
------------

The Check Fixity API endpoint that belongs to the Storage Service can be used
to trigger a fixity check on an individual package. When the Fixity client
calls this endpoint, the checking is done in the Storage Service itself. Fixity
only aggregates results and makes it easy to do this across all packages.
...
The Storage Service can detect multiple types of error. Some are shown as
...
For any error in the Fixity check the Storage Service simply keeps a summary
log, see for example when data in a file has been modified:
```sql
+--------------------------------------+----------------------------+---------+---------------+
| AIP UUID                             | Fixity Last Checked        | success | error_details |
+--------------------------------------+----------------------------+---------+---------------+
